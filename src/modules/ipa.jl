...
function (m::PointProjectionMultimer)(activations::AbstractArray, rigids::Rigid)
...
function (m::MultimerInvariantPointAttention)(s::AbstractArray, z::AbstractArray, r::Rigid, mask::AbstractArray)
    # s: (C_s, L, B), z: (C_z, L, L, B), mask: (L, B)
    q = m.linear_q(s)
    q = reshape(q, m.c_hidden, m.no_heads, size(q, 2), size(q, 3)) # (C, H, L, B)
