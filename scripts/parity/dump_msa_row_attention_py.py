...
def _extract(params, key, var):
    return np.asarray(params[f"msa_row_attention_with_pair_bias/{key}"][var], dtype=np.float32)
...
def main() -> None:
    ...
    def fn(msa_act, msa_mask, pair_act):
        mod = modules.MSARowAttentionWithPairBias(cfg, gc, name="msa_row_attention_with_pair_bias")
        return mod(msa_act, msa_mask, pair_act, is_training=False)

    transformed = hk.transform(fn)

    key = jax.random.PRNGKey(args.seed)
    k_msa, k_mask, k_pair, k_init = jax.random.split(key, 4)
    ...
    pair_act = jax.random.normal(k_pair, (args.n_res, args.n_res, args.c_z), dtype=jnp.float32)

    params = transformed.init(k_init, msa_act, msa_mask, pair_act)
    out = transformed.apply(params, None, msa_act, msa_mask, pair_act)
    ...
    arrays = {
        ...
        "feat_2d_norm_scale": _extract(params, "feat_2d_norm", "scale"),
        "feat_2d_norm_offset": _extract(params, "feat_2d_norm", "offset"),
        "feat_2d_weights": np.asarray(params["msa_row_attention_with_pair_bias"]["feat_2d_weights"], dtype=np.float32),
    ...
    print(f"msa_act={arrays['msa_act'].shape} pair_act={arrays['pair_act'].shape} out={arrays['out'].shape}")
