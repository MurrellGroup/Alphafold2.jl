...
if length(ARGS) < 1
    error("Usage: julia scripts/parity/check_msa_column_attention_jl.jl <dump.npz> [tol]")
end
...
m = MSAColumnAttention(size(msa_act, 1), num_head, head_dim)
load_msa_column_attention_npz!(m, path)
...
@printf("MSAColumnAttention parity\n")
@printf("  file: %s\n", path)
@printf("  max_abs: %.8g\n", max_abs)
@printf("  mean_abs: %.8g\n", mean_abs)
@printf("  rms: %.8g\n", rms)
