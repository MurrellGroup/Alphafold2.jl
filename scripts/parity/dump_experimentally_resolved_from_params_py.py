...
def main() -> None:
    ...
    hc = cfg.model.heads.experimentally_resolved
    ...
    c_s = int(params["experimentally_resolved_head/logits"]["weights"].shape[0])

    def fn(single):
        mod = modules.ExperimentallyResolvedHead(hc, gc, name="experimentally_resolved_head")
        return mod({"single": single}, batch=None, is_training=False)["logits"]

    transformed = hk.transform(fn)
    key = jax.random.PRNGKey(args.seed)
    key_x, key_apply = jax.random.split(key)
    single = jax.random.normal(key_x, (args.n_res, c_s), dtype=jnp.float32)
    logits = transformed.apply(params, key_apply, single)
    ...
    print(f"Saved experimentally-resolved dump: {args.out}")
