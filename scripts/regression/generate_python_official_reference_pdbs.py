#!/usr/bin/env python3
"""Generate reference PDBs using official AlphaFold RunModel codepaths."""
    ...
    for i, seq in enumerate(sequences):
        ...
        if has_templates:
            monomer_features.update(
                _build_template_features(
                    seq,
                    template_pdb=template_pdbs[i],
                    template_chain=template_chains[i],
                    residue_constants=residue_constants,
                    domain_name=f"{os.path.basename(template_pdbs[i])}_{template_chains[i]}",
                )
            )
        else:
            monomer_features.update(
...
def main() -> None:
    ...
    manifest_out = {
        ...
        "generator": "scripts/regression/generate_python_official_reference_pdbs.py",
