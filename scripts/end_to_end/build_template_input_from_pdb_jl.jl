...
function main()
    if length(ARGS) < 3
        error("Usage: julia build_template_input_from_pdb_jl.jl <template_pdb_or_csv> <template_chain_or_csv> <out.npz> [sequence] [num_recycle] [msa_file]")
    end

    pdb_arg = ARGS[1]
    chain_arg = ARGS[2]
    out_path = ARGS[3]
    seq_arg = length(ARGS) >= 4 ? strip(ARGS[4]) : ""
    num_recycle = length(ARGS) >= 5 ? parse(Int, ARGS[5]) : 1
    msa_file = length(ARGS) >= 6 ? strip(ARGS[6]) : ""

    pdb_paths = split(pdb_arg, ",")
    chain_args = split(chain_arg, ",")
    ...
    template_parsed = Vector{Tuple{String,Vector{Int32},Array{Float32,4},Array{Float32,3}}}(undef, length(pdb_paths))
    for i in eachindex(pdb_paths)
        template_parsed[i] = _parse_template_chain(strip(pdb_paths[i]), chain_args[i][1])
    end

    query_seq = if isempty(seq_arg)
        template_parsed[1][1]
    else
        uppercase(seq_arg)
    end
    ...
    NPZ.npzwrite(out_path, Dict(
        ...
        "template_aatype" => template_aatype_aligned,
        "template_all_atom_positions" => template_pos_aligned,
        "template_all_atom_masks" => template_mask_aligned,
        "num_recycle" => Int32(num_recycle),
    ))

    println("Saved template input NPZ to ", out_path)
